...
//   const { data: collectionData } =Queries.useGetCollections({ page: 1, limit: 10 });
//   const { data: seasonData } = Queries.useGetSeasons({ page: 1, limit: 10 });
//   const { data: scentData } = Queries.useGetScents({ page: 1, limit: 10 });
...
//     collectionIds: (initialValues.collectionIds || []).map(
//       (item: any) => item._id || item
//     ),

//     seasonIds: (initialValues.seasonIds || []).map(
//       (item: any) => item._id || item
//     ),

//     scentIds: (initialValues.scentIds || []).map(
//       (item: any) => item._id || item
//     ),
...
//     metaDescription: initialValues.metaDescription || "",
//     metaKeywords:
//       initialValues.metaKeywords?.length > 0
...
//     onClick={() => setForm({ ...form, isTrending: !form.isTrending })}
//     className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${
//       form.isTrending ? "bg-[var(--primary)]" : "bg-gray-300"
//     }`}
//   >
//     <span
//       className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
