...
interface BannerGalleryProps { data: any; onToggleStatus: (item: any) => void; onEdit: () => void; status: boolean;  onStatusChange: (val: boolean) => void;}
        ...
        {images.map((img: string, index: number) => (
          <div  key={index}  className="relative group rounded-xl overflow-hidden border" >
            ...
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-3">
              <button className="action-btn view" onClick={() => onToggleStatus(data)} > {data?.isActive ? <Eye size={16} /> : <EyeOff size={16} />} </button>
              <button  className="action-btn edit" onClick={onEdit} > <Pencil size={16} /> </button>
            </div>
          </div>
        ))}
