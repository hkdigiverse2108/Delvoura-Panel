...
const BannerFormPage = ({ initialValues, onSubmit, onBack }: any) => {
  ...
  useEffect(() => {
    if (initialValues) {
      setForm({
        bannerImages: initialValues.bannerImages || [],
      });
    } else {
  ...
  return (
    <CommonFormCard>
      <CommonPageHeaderForm
        title={isEdit ? "Edit Banner" : "Manage Banner"}
        description={
          isEdit
            ? "Update your banner images"
            : "Add or update banner images for your store"
        }
        onBack={onBack}
      />
        ...
        <div className="space-y-8">
          <div>
            <MultipleImageField
              ...
              required
              onChange={(urls) => {
                setForm({ ...form, bannerImages: urls });
                setErrors((prev: any) => ({ ...prev, bannerImages: "" }));
              }}
            />
            {errors.bannerImages && (
          ...
          <CommonFormActions
            ...
            disabled={!form.bannerImages.length}
            loading={loading}
          />
        </div>
      </div>
