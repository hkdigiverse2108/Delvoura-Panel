...
const Season = () => {
  ...
  const handleSubmit = async (values: any) => {
    if (editData) {
      await updateSeason.mutateAsync({ ...values, seasonId: editData._id,});
    } else {
      await addSeason.mutateAsync(values);
    }
  ...
  if (mode === "form") {
    return (
      <SeasonFormPage initialValues={editData} onSubmit={handleSubmit} onBack={() => { setMode("list");  setEditData(null); }}
      />
    );
  }
  ...
  return (
    <div className="user-page">
      <CommonPageHeader
        title={PAGE_TITLE.SEASON.TITLE}
        subtitle={PAGE_TITLE.SEASON.SUB_TITLE}
        buttonText={PAGE_TITLE.SEASON.BUTTON_TEXT}
