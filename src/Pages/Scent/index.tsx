...
const Scent = () => {
  ...
  const { data, isLoading, refetch } =
    Queries.useGetScents(queryParams);
  ...
  const handleSubmit = async (values: any) => {
    try {
      if (editData) {
        await updateScent.mutateAsync({
          ...values,
          scentId: editData._id,
        });
      } else {
        await addScent.mutateAsync(values);
      }
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Delete this scent?")) {
      await deleteScent.mutateAsync(id);
      refetch();
    }
  };
  ...
  return (
    <div className="user-page">
      <CommonPageHeader
        title="Scents"
        subtitle="Manage fragrance types"
        buttonText="Add Scent"
      ...
      <CommonPagination
        ...
        limit={pagination.limit}
        total={total}
        currentCount={filteredScents.length}
        label="scents"
        onPageChange={(page) =>
